...
(function () {
    axios.get(`https://api.monobank.ua/bank/currency`)
        .then(response => {
            ...
            data.forEach(pair => {
                let { currencyCodeA, rateBuy, rateSell, rateCross } = pair;
                sql.query(`select id from currency_pairs where base_currency='UAH' and currency = (select code from currencies where number_code = ${currencyCodeA})`, (err, result) => {
                    ...
                    sql.query(`update exchange_rates set (sale_mono, purchase_mono) = (${rateSell ? rateSell : 'null'}, ${rateBuy ? rateBuy : 'null'}) where currency_pair_id = ${result.rows[0].id} and date = '${date}'`, (err, result) => {
                        if (err)
